...
public class ClientPortGUI extends JOptionPane {
    ...
    private int selectPort() {
        try {
            String portStr = (String) showInputDialog(client.getChatMessanger().getChatGUI(), "", "포트 번호 입력", PLAIN_MESSAGE, null, null, ChatMessangerData.getPortData());
            if (portStr != null) {
                int port = Integer.parseInt(portStr);
                if (port >= 0 && port <= 65535) {
                    return port;
                }
                client.getChatMessanger().getChatGUI().getOutputScrollPane().getOutputTextArea().appendText("숫자만 입력하세요 (범위: 0~65535)");
                return selectPort();
            } else {
