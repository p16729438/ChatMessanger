    ...
    private void inputData(String str) {
        try {
            if (str.split(";", -1)[0].equalsIgnoreCase("chat")) {
                ...
                connectThread.getClient().getChatMessanger().output(str.split(";", -1)[1] + str.split(";", -1)[2] + ": " + chat);
                ...
                if (connectThread.isRequestingNickname()) {
                    if (str.split(";", -1)[2].equalsIgnoreCase("true")) {
                        ...
                        if (str.split(";", -1)[3].equalsIgnoreCase("twice")) {
                            connectThread.getClient().getChatMessanger().output(str.split(";", -1)[1] + "닉네임은 한 번만 설정할 수 있습니다.");
                ...
                if (connectThread.getClient().getChatMessanger().usingGUI()) {
                    connectThread.getClient().getChatMessanger().getChatGUI().getClientListScrollPane().getClientList().addClient(nickname);
                }
                ...
                String nickname = str.split(";", -1)[2];
                if (connectThread.getClient().getChatMessanger().usingGUI()) {
                    connectThread.getClient().getChatMessanger().getChatGUI().getClientListScrollPane().getClientList().removeClient(nickname);
                }
                ...
                if (connectThread.getClient().getChatMessanger().usingGUI()) {
                    for (int i = 2; i < str.split(";", -1).length; i++) {
                        connectThread.getClient().getChatMessanger().getChatGUI().getClientListScrollPane().getClientList().addClient(str.split(";", -1)[i]);
                    }
                } else {
                    int clientCount = 0;
                    ...
                    connectThread.getClient().getChatMessanger().output("현재 접속중 (" + clientCount + "): " + clientListStr);
                }
