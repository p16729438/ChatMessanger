...
public class ClientInputThread extends Thread {
    ...
    public void run() {
        try {
            ...
            dis.close();
            connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane().getOutputTextArea()
                    .appendText("Server: 연결 종료");
    ...
    private void inputData(String str) {
        try {
            if (str.split(";", -1)[0].equalsIgnoreCase("chat")) {
                ...
                connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane().getOutputTextArea()
                        .appendText(str.split(";", -1)[1] + str.split(";", -1)[2] + ": " + chat);
                ...
                if (connectThread.isRequestingNickname()) {
                    if (str.split(";", -1)[2].equalsIgnoreCase("true")) {
                        ...
                        if (str.split(";", -1)[3].equalsIgnoreCase("twice")) {
                            connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane()
                                    .getOutputTextArea().appendText(str.split(";", -1)[1] + "닉네임은 한 번만 설정할 수 있습니다.");
                        } else if (str.split(";", -1)[3].equalsIgnoreCase("length")) {
                            connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane()
                                    .getOutputTextArea()
                                    .appendText(str.split(";", -1)[1] + "닉네임이 너무 짧거나 깁니다. (1자~16자)");
                            connectThread.clearRequestNickname();
                        } else if (str.split(";", -1)[3].equalsIgnoreCase("invalid")) {
                            connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane()
                                    .getOutputTextArea().appendText(str.split(";", -1)[1]
                                            + "닉네임에 사용할 수 없는 글자가 포함되어 있습니다. (\"Server\", \"null\", \";\", 공백, 줄바꿈 사용 불가)");
                            connectThread.clearRequestNickname();
                        } else if (str.split(";", -1)[3].equalsIgnoreCase("number")) {
                            connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane()
                                    .getOutputTextArea().appendText(str.split(";", -1)[1] + "닉네임은 숫자로만 이루어질 수 없습니다.");
                            connectThread.clearRequestNickname();
                        } else if (str.split(";", -1)[3].equalsIgnoreCase("already")) {
                            connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane()
                                    .getOutputTextArea().appendText(str.split(";", -1)[1] + "이미 존재하는 닉네임입니다.");
                            connectThread.clearRequestNickname();
                        } else {
                            connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane()
                                    .getOutputTextArea().appendText("알 수 없는 형식의 정보 수신됨: " + str);
                        }
                    } else {
                        connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane()
                                .getOutputTextArea().appendText("알 수 없는 형식의 정보 수신됨: " + str);
                    ...
                    connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane().getOutputTextArea()
                            .appendText("알 수 없는 형식의 정보 수신됨: " + str);
                }
                ...
                connectThread.getClient().getChatMessanger().getChatGUI().getClientListScrollPane().getClientList()
                        .addClient(nickname);
                connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane().getOutputTextArea()
                        .appendText(str.split(";", -1)[1] + nickname + ": 연결됨");
            } else if (str.split(";", -1)[0].equalsIgnoreCase("disconnectinfo")) {
                String nickname = str.split(";", -1)[2];
                connectThread.getClient().getChatMessanger().getChatGUI().getClientListScrollPane().getClientList()
                        .removeClient(nickname);
                connectThread.getClient().getChatMessanger().getChatGUI().getOutputScrollPane().getOutputTextArea()
                        .appendText(str.split(";", -1)[1] + nickname + ": 연결 종료");
